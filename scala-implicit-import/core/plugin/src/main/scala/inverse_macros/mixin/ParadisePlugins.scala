...
trait ParadisePlugins extends org.scalamacros.paradise.typechecker.AnalyzerPlugins with UseGlobal {
    ...
    override def pluginsEnterSym(namer: NscNamer, tree: Tree) = {
      tree match {
        ...
        case ddef@DefDef(mods, _, _, _, _, _)
          ...
          val d = ddef.copy(mods = mods.withAnnotations(List(
            q"new inverse_macros.inverseMacroEngine()", q"new inverse_macros.IMEngineApplied()")))
